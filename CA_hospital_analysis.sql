...
-- -- Top 5 Counties in dataset
-- SELECT county, performance_measure, SUM(cases) AS total_cases
-- FROM ca.perf
-- WHERE county IN ('Los Angeles', 'Orange', 'San Diego', 'Riverside', 'San Bernardino')
-- GROUP BY county, performance_measure
-- ORDER BY total_cases DESC;

-- -- grouping sets by year, county, and adverse events (undesirable events associated with hospital care)
-- SELECT year, county, SUM(adverse_events) AS "Total Adverse Events"
-- FROM ca.perf
-- WHERE county IN ('Los Angeles', 'Orange', 'San Diego', 'Riverside', 'San Bernardino')
-- GROUP BY GROUPING SETS (year), county
-- ORDER BY year;

-- every county outside of top 5 counties
SELECT year, county, SUM(adverse_events) AS "Avg Adverse Events"
FROM ca.perf
WHERE NOT county IN ('Los Angeles', 'Orange', 'San Diego', 'Riverside', 'San Bernardino')
GROUP BY GROUPING SETS (year), county
ORDER BY year;
